---
date: 2024-10-14
tags:
  - Journal
  - Meeting
---

## Attendees

- Hesti/Viability: [[Adria Tarrida|Adria]], [[Henry]]
- AMRC: [[JS]], [[Nacho]], [[Sarah]], [[Rhys]]
- [[Benjamin Ford]] - Monitoring Service Provider for IUK
- Note: [[Tom]] (senior software engineer, working on this grant for Hesti) was not on the call.

## Purpose

- Innovate UK (IUK) kick-off meeting for the Hesti/Viability project.

## Key Discussion Points

- "Viability" is a rebranding of Hesti; Hesti Ltd remains the registered company.
- This work continues what was previously done under the RTO grant between Viability and AMRC.
- [[Benjamin Ford]] presented slides covering the basics/formalities of starting an IUK project, including Raid-app.io, a rail planning tool from a previous IUK project.
- Overview of potential outstanding actions identified by [[Benjamin Ford]] - mostly already dealt with.
  - Spend profile shouldn't be flat, but this is less critical for a six-month project; Viability has an upfront cost with Ordnance Survey.
  - A detailed risk review is required soon, at the first project meeting.
    - Commercial risks are light on the ground.
    - Regulatory compliance - constraints applied to models to ensure policies are complied with.
- Dissemination activities currently just boilerplate items.
- Discussion of which events to present at - AMRC AI Showcase, Connected Places Catapult.

## Decisions

- None recorded.

## Action Items

- (Team) - (Conduct detailed risk review at first project meeting)
- (Team) - (Consider presenting at AMRC AI Showcase / Connected Places Catapult)

---
date: 2020-11-24
tags:
  - Journal
  - Meeting
---

## Attendees

- [[JS]]
- [[Rob Poyner|Rob]]
- [[Alan Norbury|Alan]]
- [[Nat]]

## Purpose

## Key Discussion Points

- To bring up: the ADAS issue - do [[Nat]], [[Rob Poyner|Rob]] or [[Alan Norbury|Alan]] have any other contacts? Already contacted [[Charles Tivey]] and [[Siemens]] forums.
- [[Nat]] has sent over a cyber questionnaire with details.
- SFIH:
  - [[Alan Norbury|Alan]] keen to support - just needs to know what the plan is and what support is needed.
  - LOGO for legacy lathe and mill?
  - IoT24.
  - RFID:
    - RTLS - a bit like Ubisense but without the calibration requirement (active).
    - Passive antennas can now work at up to 8m (Congleton example).
    - QR and barcode reading.
    - Fairfield Labels - [[Siemens]]' tech partner to go to market.
  - TCPU.

## Decisions

- ...

## Action Items

- Person - (Action)

---
date: 2025-03-24
tags:
  - Journal
  - Meeting
---

## Attendees

- [[JS]] (attending a series of one-to-one meetings/networking at a Birmingham event)
- MayaHTT - [[Marc Lafontaine]]
- AutoAlign [[AI]] - [[Dan Adamson]], [[Mike Knobben]]
- Vooban - [[Hugues Foltz]] (Exec VP)
- DaoAI - did not meet
- NTWIST - did not meet, but spoke to someone from them during lunch

## Purpose

- Series of one-to-one style meetings with visiting Canadian [[AI]] companies to scope out collaboration potential.

## Key Discussion Points

- **MayaHTT ([[Marc Lafontaine]])**: Very experienced company, 150+ projects in [[IIoT]]/[[AI]] space. [[Siemens]] Insights Hub plug-in/app; Smart Machine Assistant is an [[Machine Learning|ML]]-based system relying on [[Siemens]] hardware for data capture from machine tools, providing optimised parameters for operators (advisory only, doesn't control). UK presence for ~6 years via long relationship with [[Rolls-Royce]]; Marc personally involved with several [[Rolls-Royce|R-R]] business areas. Doing physics-based [[AI]] work. Potentially useful for the Composites team - Maya have done fibre orientation work. Open to R&D collaborations and could be a good outlet/integrator for AMRC R&D that needs a next step.
- **AutoAlign [[AI]] ([[Dan Adamson]], [[Mike Knobben]])**: [[AI]] supervisor that interacts with [[LLM|LLMs]] for safe, secure, reliable, and compliant dynamic interaction with [[LLM|LLMs]]. Emphasis on "[[AI]] Augmented workforce" rather than replacement of people with [[AI]]. Context-aware [[AI]] examples in wearables that understand a worker's current task context. Multimodal data angle - potential link-up with Hai Ping and UKOMAIN. Very open to R&D collaborations and signposting opportunities.
- **Vooban ([[Hugues Foltz]])**: Tailored [[AI]] solutions / digital transformation. Met Hugues over lunch - discussed shared UK/Canada challenges with slow [[AI]] adoption. Strong opinions: e.g. EU [[AI]] Act seen as stifling progress/adoption vs the US approach which is closer to forcing adoption. Vooban has ~200 people but is considered an [[AI]] superpower in Canada, competing with KPMG, Accenture etc. - Hugues sees this as illustrative of how slow the [[AI]] world still is in Canada. Vooban runs an annual event (possibly "Insights [[AI]]") featuring industry speakers discussing [[AI]] implementation journeys - Hugues couldn't find an equivalent in the UK, a possible joint opportunity. Somewhat open to research collabs with Vooban as tech provider. Looking to open a UK office/team soon.
- **DaoAI** (not met): [[AI]]-driven inspection and defect detection; AOI (automated optical inspection).
- **NTWIST** (not met, spoke to someone at lunch): nScheduler product covers monitoring, predictive maintenance, adaptive scheduling, energy management. Specialise in HVLV (High Variety, Low Volume) sectors.
- Questions JS wanted to explore across these companies: common challenges faced, data availability, which manufacturing sectors they target, existing UK presence, and appetite for R&D collaboration.

## Decisions

- None recorded - these were exploratory/networking conversations rather than formal decision-making meetings.

## Action Items

- [[JS]] - Follow up on common ground across companies re: challenges faced, data availability, target manufacturing sectors, existing UK presence, and R&D collaboration appetite.
- [[JS]] - Consider potential link between MayaHTT ([[Marc Lafontaine]]) and the Composites team on fibre orientation work.
- [[JS]] - Consider potential link between AutoAlign [[AI]] and Hai Ping / UKOMAIN on multimodal data.

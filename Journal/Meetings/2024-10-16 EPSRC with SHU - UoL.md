---
date: 2024-10-16
tags:
  - Journal
  - Meeting
---

## Attendees

- [[Arutian Ehiasarian|Harry]]
- [[James Bradley|James]]
- [[JS]]

## Purpose

- Continued work on the EPSRC proposal with [[Sheffield Hallam University|SHU]]/UoL, refining case for support, ethics, and data-sharing sections.

## Key Discussion Points

- [[Arutian Ehiasarian|Harry]] has reduced his costs so the proposal is now within budget.
- R4RI - [[JS]] to update the section on leading others and wider contribution.
- Case for support:
  - Challenges: finding and engineering data, data management, data strategy.
  - Novelty - review inclusion of the [[AMRC Data Cloud]].
  - Contribution to knowledge - clarify what's new from the data/digital twin/[[Machine Learning|ML]] side; look at feedback from the previous application.
  - [[Arutian Ehiasarian|Harry]] has updated the case to focus on process efficiency.
  - Check WP1 based on [[Arutian Ehiasarian|Harry]]'s updates.
  - WP4 - check the low-carbon opportunity is sufficiently covered.
  - Ensure sustainability language matches the call's specific wording (see call text: https://www.ukri.org/opportunity/research-to-drive-the-next-generation-of-manufacturing-technologies/).
  - Potential for commercialisation - maybe send ideas to [[Arutian Ehiasarian|Harry]]; eventual software solution envisaged.
- JoR (Justification of Resources) might need updating.
- [[AI Ethics|Ethics]]:
  - Data sharing to cloud platforms - not making data public from factories.
  - Re-use of data - sharing via ORDA for experimental data that is not commercially sensitive; seek permission to share data not generated by the consortium.
  - Work closely with data owners (e.g. Pilkington); obtain consent from data owners for data harvesting activities.
  - Draft ethics text: primary data will come from experimental work at [[Sheffield Hallam University|SHU]] and industrial collaborator sites, cleaned/engineered by the research team, and made available to other researchers via a service such as [[University of Sheffield]]'s ORDA where not commercially sensitive, with publications referencing the data archive.
  - Sharing of primary data from industrial collaborators will require rigorous review of commercial sensitivity, with permission sought from data providers; shareable data will be anonymised and stripped of commercially sensitive information/identifiers as directed.
  - Secondary source data will be collected from existing curated data archives for model training/validation, checked to ensure any conditions imposed by data authors (e.g. attribution) are followed, or excluded if conditions are too prohibitive.

## Decisions

- Case for support to focus on process efficiency, per [[Arutian Ehiasarian|Harry]]'s updates.
- Data ethics approach agreed: no public factory data, use of ORDA for shareable experimental data, anonymisation of any shareable industrial data, and permission-seeking for reuse of secondary sources.

## Action Items

- [[JS]] - (Update R4RI section on leading others and wider contribution)
- (Team) - (Check WP1 and WP4 low-carbon content against [[Arutian Ehiasarian|Harry]]'s updates)
- (Team) - (Review call text for required sustainability wording)
- (Team) - (Send commercialisation ideas to [[Arutian Ehiasarian|Harry]])

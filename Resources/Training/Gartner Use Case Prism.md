---
source: https://www.gartner.com/document/4382599
tags:
  - Resource
  - Training
...
- The **Use-Case Prism Categories** diagram plots use cases on a triangular prism with two axes running from the base up to the apex: **Business Value** (left side, Low → Medium → High) and **Feasibility** (right side, Low → Medium → High). The prism is divided into four diamond-shaped categories:
  ...
  - **Calculated Risks** (left side of the prism): use cases offering **high business value but low feasibility**, meaning they represent riskier options.
  - **Marginal Gains** (right side of the prism): use cases that are **highly feasible but offer low business value**, making them low risk but for marginal gain.
  - **Selective Exceptions** (bottom of the prism): use cases offering **low business value and low feasibility**, making them lower priority unless in select circumstances.
- Each use case is placed into one of these four categories **based on its position on the Prism**.
